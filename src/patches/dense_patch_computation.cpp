        ...
        Router& router)
{
    ...
    if (const auto* bellprep = std::get_if<LocalInstruction::BellPrepare>(&instruction.operation))
    {
        ...
        bell_state.push_back(LayoutHelpers::basic_square_patch(bellprep->cell1));
        bell_state.push_back(LayoutHelpers::basic_square_patch(bellprep->cell2));
        bell_state[0].id = bellprep->side1; bell_state[1].id = bellprep->side2;

        slice.place_sparse_patch(bell_state[0], false);
        slice.place_sparse_patch(bell_state[1], false);
    ...
    else if (const auto* bellmeas = std::get_if<LocalInstruction::BellMeasure>(&instruction.operation))
    {
        ...
        slice.patch_at(bellmeas->cell1)->activity = PatchActivity::Measurement;
        slice.patch_at(bellmeas->cell2)->activity = PatchActivity::Measurement;
    ...
    else if (const auto* move = std::get_if<LocalInstruction::Move>(&instruction.operation))
    {
        ...
        {
            return {std::make_unique<std::runtime_error>(lstk::cat("Patch at ", move->cell1, " is active")), {}};
        }
        else if (!slice.is_cell_free(move->cell2))
        {
            return {std::make_unique<std::runtime_error>(lstk::cat("Cell ", move->cell2, " is not free")), {}};
        }
        SparsePatch patch = LayoutHelpers::basic_square_patch(move->cell2);
        patch.id = move->target;
        slice.place_sparse_patch(patch, false);
        slice.get_boundary_between(move->cell1,move->cell2)->get().is_active=true;
        slice.get_boundary_between(move->cell2,move->cell1)->get().is_active=true;
        slice.patch_at(move->cell1)->activity = PatchActivity::Measurement;

        return {nullptr, {}};
    }
    // TRL 04/04/23: Implementing TwoPatchMeasure LocalInstruction
    else if (const auto* localmeas = std::get_if<LocalInstruction::TwoPatchMeasure>(&instruction.operation))
    {
        ...
        if (slice.get_boundary_between(localmeas->cell1, localmeas->cell2).has_value())
        {
            slice.get_boundary_between(localmeas->cell1,localmeas->cell2)->get().is_active=true;
            slice.get_boundary_between(localmeas->cell2,localmeas->cell1)->get().is_active=true;

            return {nullptr, {}};
        }
        else
        {
            throw std::logic_error("LocalInstruction::TwoPatchMeasure called on a boundary that does not exist");
        }
    }
        ...
        Router& router)
{
    ...
    else if (const auto* m = std::get_if<MultiPatchMeasurement>(&instruction.operation))
    {
        ...
        // TRL 04/04/23: logic depending on local_instructions
        ...
        {
            //TRL 04/04/23: For now, construct TwoPatchMeasure instruction and apply it; later might want to generalize to break up longer ops
            {
    ...
    else if (auto* bell_init = std::get_if<BellPairInit>(&instruction.operation)) 
    {
        ...
        if (!local_instructions)
        {
            ...
            // TRL 04/04/23: May be worth returning to insert Pauli corrections
            // TRL 04/04/23: It also looks like the boundary between logical qubits and routing region is colored in 
        }
        else 
        {
            ...
            if (!bell_init->counter.has_value())
            {
                ...
                // TRL 03/30/23: Generalizing for case of routing region size of 2
...
{
    // TRL 04/04/23: Using compilation flag
    InstructionApplicationResult r = try_apply_instruction_direct_followup(slice, instruction, local_instructions, layout, router);
    if (r.maybe_error && r.followup_instructions.empty())
        ...
        DensePatchComputationResult& res)
{
    ...
    while (instruction_stream.has_next_instruction() || !future_instructions.empty())
    {
        ...
        // TRL 04/04/23: Using compilation flag
        auto application_result = try_apply_instruction_with_followup_attempts(slice, instruction, local_instructions, layout, router);
    ...
    while (!dag.empty())
    {
        ...
        for (dag::label_t instruction_label: non_proximate_instructions)
        {
            ...
            else
            {
                res.ls_instructions_count_++;
                instruction_visitor(instruction);
                // TRL 04/13/23: Handle follow-up instructions
                handle_followup_instructions(instruction_label, std::move(application_result.followup_instructions));
            }
        }
